...
struct FirstStepView: View {
    ...
    let audioPlayer = AVPlayer(url: Bundle.main.url(forResource: "First Step", withExtension: "mp3")!)
    ...
    var body: some View {
        ...
        .onAppear {
            startTimer()
            audioPlayer.play()
            HealthModel.shared.startToothbrushing()
        }
        .onChange(of: timeRemaining) { newValue in
            if newValue == 0 {
//                print("masuk kesini nih")
                activePage = .secondStep
                audioPlayer.pause()
            }
        }
...
struct FirstStepView_Previews: PreviewProvider {
    static var previews: some View {
        FirstStepView(activePage: .constant(.firstStep))
    }
}
//dummy buat munculin view
